...
/// `String` on every request or response.
...
/// True when `buf` is empty or only ASCII whitespace.
pub fn line_is_blank(buf: &[u8]) -> bool {
...
mod tests {
    ...
    #[test]
    fn limited_line_into_reuses_and_strips_crlf() {
        let mut cursor = Cursor::new("a\r\nb\n");
        let mut buf = Vec::with_capacity(8);
        assert!(read_limited_line_into(&mut cursor, &mut buf, 16).unwrap());
        assert_eq!(&buf, b"a");
        let cap = buf.capacity();
        assert!(read_limited_line_into(&mut cursor, &mut buf, 16).unwrap());
        assert_eq!(&buf, b"b");
