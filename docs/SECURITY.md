...
| Token | Required by default. Non-empty `GPUI_AGENT_TOKEN` is used as-is. If unset, `from_env` mints a 32-byte hex token and the host prints it once after bind. `GPUI_AGENT_ALLOW_EMPTY_TOKEN=1` restores optional-token labs. `AgentServer::bind(..., None)` stays allowed for in-process tests. |
...
socket **and** present the token can drive the UI as the user — snapshot
field values, click, `invoke`, `shutdown`. On a single-user developer
machine that is the point. On a shared host it is a cross-user confused
deputy. The default ephemeral/env token narrows that to “whoever knows
the secret,” but the secret is still a local env var (or stderr once).
...
| --- | --- | --- | --- | --- | --- | --- |
| H1 | **High** | Design | Token used to be optional. With `GPUI_AGENT=1` and no `GPUI_AGENT_TOKEN`, **any local process** could snapshot, click, invoke, and shut down the app. | Former `security.rs` `from_env` treated empty/missing token as `None`. `handle_request` skips auth when `expected_token` is `None`. | Malware, another user on the same box, or a compromised MCP client talks to `127.0.0.1:17421` and drives the UI / reads field values. | **Patched (P2, policy D).** `from_env` mints a 32-byte hex token when `GPUI_AGENT_TOKEN` is unset and prints it once after bind. CLI/MCP/`recipe run` refuse to connect without a token. Opt-out is explicit: `GPUI_AGENT_ALLOW_EMPTY_TOKEN=1` and/or `--allow-empty-token` (must match on host and client). Empty `GPUI_AGENT_TOKEN=` is treated as unset (mint), not as opt-out. `bind(..., None)` remains for tests. `./scripts/smoke.sh` exports a shared `GPUI_AGENT_TOKEN`. Unix sockets (M2) are still later. |
| H2 | **High** | Bug | Unbounded NDJSON lines + one OS thread per connection. A client could grow a request line without limit (`BufRead::lines`) and/or open unbounded handler threads. | Former `server.rs` `reader.lines()` and `thread::spawn` on every `accept`. | Local process (no token needed if the host is `Open`) sends a multi-GB line or opens thousands of connections → memory / thread exhaustion of the GUI process. | **Patched.** `read_limited_line` (default 1 MiB), `MAX_CONNECTIONS` (32), idle read/write timeout (30s). Extra clients are dropped. Oversized lines get an error and the socket is closed (no resync). |
...
| L5 | **Low** | Design | Token lives in the process environment (`GPUI_AGENT_TOKEN`), visible via `/proc/<pid>/environ` and some `ps` invocations. Ephemeral tokens are also printed once to stderr. | `from_env`, clap `env = "GPUI_AGENT_TOKEN"`, `eprint_token_banner` | Local attacker with the same or root uid reads the secret. Same class as H1/M2. | **Documented.** Unix-socket + file secret (0600) is still the real multi-user fix (M2). Ephemeral print is the Jupyter model. |
...
`authorize_request` is the single gate (version + token when the host
has one). Used by `handle_request` and by both TCP stream handlers
**before** mailbox post / host dispatch. Comparison is `tokens_match`.
Failures close the socket. Hosts started via `from_env` always have a
token unless `GPUI_AGENT_ALLOW_EMPTY_TOKEN=1`. CLI connecting commands
refuse to start without `--token` / `GPUI_AGENT_TOKEN` unless
`--allow-empty-token`.
...
| Token / version | Every step is a normal `Request`. `authorize_request` still runs. Missing or **wrong** token fails the step; the server still closes. CLI `recipe run` and MCP `recipe_run` also refuse to start without a client token unless `--allow-empty-token`. |
...
Startup logs print the bind address. Env-provided tokens are never
echoed. Ephemeral tokens are printed **once** after bind as
`GPUI_AGENT_TOKEN=…`. Responses do not echo the token. `Debug` for
`Request` / `AgentConfig` redacts it. Do not log raw request lines.
...
`gpui-agent` itself depends on `serde`, `serde_json`, `thiserror`, and
`getrandom` (ephemeral token mint). The CLI adds `anyhow` + `clap`.
`cargo audit` at review time reported
...
### P2 (required / ephemeral token)

- `from_env` mints a 32-byte hex token when `GPUI_AGENT_TOKEN` is unset.
- CLI/MCP/`recipe run` refuse to connect without a token unless
  `--allow-empty-token` / `GPUI_AGENT_ALLOW_EMPTY_TOKEN=1`.
- Sample hosts call `eprint_token_banner` after bind (ephemeral token
  printed once; env token never echoed).
- `getrandom` for mint entropy. `bind(..., None)` still valid for tests.
- `./scripts/smoke.sh` exports `GPUI_AGENT_TOKEN=gpui-agent-smoke`.
...
1. **Unix-domain socket + peer uid (M2).** Optional
...
7. **MCP `Content-Length` framing.** The shim is newline JSON. Spec
...
8. **CI: `cargo test` + `cargo audit`.** Generate or commit a lockfile
   so audits are reproducible. Fail on vulnerability advisories; warn
   on unmaintained.
9. **Default `agent` feature off in app templates (I3).** Keep it on
    for `apps/todo` (this is a lab) but say so in INTEGRATING.md as a
    copy-paste trap.
10. **Mailbox + token + virtual integration test.** Needs a GPU/display
    or a fake `Window`. Until then, keep the unit gates
    (`authorize_request` on virtual ops, mailbox overflow).
11. **`screenshot` on desktop** when a GPU is present (already on the
    README next-steps list).
12. **AccessKit auto-export** so apps register fewer ids by hand.
13. **Per-connection QPS cap** if anyone runs this as a long-lived
    host. Line/connection/mailbox caps are enough for v1.
14. **Invoke allow-lists in docs per app.** Sample todo is CRUD-only;
    integrators must not map `invoke` onto a shell.

## Recommended (not implemented here)

Items 1–3 and 8 are the remaining security follow-ups. Do not enable the bridge
