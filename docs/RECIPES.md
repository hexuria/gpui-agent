...
This is an **experiment** stacked on P0 session reuse
([PR #6](https://github.com/hexuria/gpui-agent/pull/6)). It does not
replace semantic RPC, does not steal the OS pointer, and does not change
protocol v1.
...
JSON is the **canonical** format. `.wants` is a thin line-based alias
that compiles to the same `Recipe` struct (not a second product). Say
if you want `.wants` deleted.

Threat model vs existing caps: [SECURITY.md](SECURITY.md#recipes-experimental).
...
Session reuse, NDJSON buffers, flatten, and mailbox `take` landed in
P0 ([PERF.md](PERF.md)). This crate adds compile/validate of recipes
on top of that session. Execution is **sequential** `rpc` on one
socket — no `rpc_pipeline`.
...
crates/gpui-agent          Session reuse (P0)
