...
`main`; this tree includes **P3** Mac window PNG and **P4** CI recipe receipt).
...
# headless / Linux / Windows: receipt lists screenshot_unavailable (no files)
# macOS desktop todo: real PNG of this window (Screen Recording)
gpui-agent recipe run examples/recipes/todo-crud.json --set title="Buy milk" \
...
receipt. Headless (and Linux/Windows desktop GPUI) returns
`screenshot_unavailable` and **does not invent a file**. macOS desktop
`todo` writes **this window** via `screencapture -l`. See
...
| No OS HID | `delivery` defaults to `semantic`. `virtual` is still in-process GPUI. `--screenshot-dir` is observe-only (macOS: this window; never the desktop). |
...
8. **Real desktop PNG (P3 — this PR).** Headless stays honest
   (`screenshot_unavailable`). macOS desktop uses `screencapture -l`.
   `--record` / SVG+PPM / ScreenCaptureKit stay later.
